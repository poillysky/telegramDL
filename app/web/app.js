...
function classifyLogText(text) {
  ...
  // Real failures only
  if (
    /任务失败|下载失败|自动恢复失败|合并失败|修复失败|占位失败|未登录|异常|不符|size mismatch|incomplete|Errno|No such file/i.test(
      t
    )
  ) {
    return "err";
  }
  if (/失败|错误/.test(t)) return "err";
  if (
    /已下载|任务完成|成功|已合并|已重新连接|登录成功|已下完|跳过|无需清理|已同步|已启用/.test(
      t
    )
  ) {
    return "ok";
  }
  if (/暂停|测试|限流|FloodWait|等待|上限|重试|待命/.test(t)) return "warn";
  if (
    /正在下载|开始下载|启动|继续|优先重试|索引扫描|文案索引|建索引|断点续传|下载标签|进入监控|开始补下|切换补下|待补下/.test(
      t
    )
  ) {
    return "busy";
...
function humanizeLogText(text) {
  ...
  t = t.replace(/^重试失败\s+(\d+)\s*条$/, "重试未完成 $1 条");
  t = t.replace(
    /仍有\s+(\d+)\s*条失败，已暂停/,
    "仍有 $1 条未完成，已暂停"
  );
